...
void client_task(void* pvParameters) {
	...
	// Open database
	char db_name[32];
	snprintf(db_name, sizeof(db_name)-1, "%s/temp.db", base_path);
	sqlite3 *db;
	sqlite3_initialize();
	...
	while(1) {
		...
		if (cJSON_GetObjectItem(root, "id")) {
			...
			if ( strcmp (id, "init") == 0) {
				const char *sql = "SELECT * FROM temperature ORDER BY timestamp DESC LIMIT 10;";
                char errmsg[128];
                sock_db_exec(db, sql, errmsg);
			} // end of init
