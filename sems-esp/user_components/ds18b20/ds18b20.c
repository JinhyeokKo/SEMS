...
static void ds18b20_read_temp(void *arg)
{
    ...
    while (retry < max_retries && !reset_success)
    {
        if (reset() == ESP_OK)
        {
            reset_success = true;
        ...
        {
            retry++;
            ESP_LOGI(TAG, "DS18B20 온도 변환 시작 대기 중... (%d/%d)", retry,
                     max_retries);
    ...
    while (retry < max_retries && !reset_success)
    {
        ...
        {
            retry++;
            ESP_LOGI(TAG, "DS18B20 온도 읽기 대기 중... (%d/%d)", retry,
                     max_retries);
...
{
    const esp_timer_create_args_t timer_args = {.callback = &ds18b20_read_temp,
                                                .name     = "ds18b20_timer"};
